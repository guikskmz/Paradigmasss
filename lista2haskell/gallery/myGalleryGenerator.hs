...
--exercicio 1) completar usando função de alta ordem:

genContent :: [(String,String)] -> String
genContent items = concat(map (imageDiv) items) -- alta ordem (map)


--exercicio 2) mesma função com list comprehension:

genContent2 :: [(String,String)] -> String
genContent2 items = foldr1 (++) [imageDiv items | items <- items] -- redução, list comprehension
